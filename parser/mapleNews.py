...
def get_data(url, feature):
    ...
    if feature:
        news_items = soup.select(
            "body > div.global-content > div.section.section--ghosted-grey > div > div > div.component.component-featured-news > ul > li"
        )
        for item in news_items:
            ...
            link = (
                ...
                + item.select_one("div.text > h3 > a")["href"]
            )
        ...
        news_items = soup.select(
            "body > div.global-content > div.component.component-news-filterable > div > div > div.news-wrapper > ul > li"
        )
        for item in news_items:
            title = item.select_one("div.text > h3 > a").text
            content = item.select_one("div.text > p:nth-child(2)").text
